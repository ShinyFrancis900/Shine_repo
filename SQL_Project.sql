...
*Explanation.     The SQL query consists of three separate "SELECT" statements.
                  Each "SELECT" statement retrieves data from a different table: "Customers_new", "Transactions_new", and "prod_cat_info".
...
*Explanation.     The "SELECT" statement is used to retrieve data from the database.
                  ...
                  "FROM" Transactions specifies the table from which to retrieve the data, in this case, the "Transactions_new" table.
...
*Explanation.    The "DATEDIFF" function is used three times to calculate the difference between the minimum (MIN(tran_date)) and maximum (MAX(tran_date)) transaction dates.
		 ...
		 This Query calculates the total number of days, months, and years between the earliest and latest transaction dates in the "Transactions_new" table, providing insight into the time range covered by the transaction data.
...
*Explanation.        The "SELECT TOP 1" statement is used to retrieve only the top 1 row from the result set.
                     ...
                     "GROUP BY" is a Clause which groups the rows of the "Transactions_new" table by the values in the "Store_type" column.
